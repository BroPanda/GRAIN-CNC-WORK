...
CREATE TABLE IF NOT EXISTS users (
  ...
  -- категорії сповіщень для Telegram через кому; 'all' = усі, порожньо = жодних
  tg_buckets        TEXT    NOT NULL DEFAULT 'all',
  -- надсилати в Telegram і власні дії: для власника це журнал усього, що в цеху
  tg_self           INTEGER NOT NULL DEFAULT 0,
...
-- які категорії сповіщень дублювати в Telegram; 'all' = усі, порожньо = жодних
ALTER TABLE users ADD COLUMN IF NOT EXISTS tg_buckets TEXT NOT NULL DEFAULT 'all';
ALTER TABLE users ALTER COLUMN tg_buckets SET DEFAULT 'all';
ALTER TABLE users ADD COLUMN IF NOT EXISTS tg_self INTEGER NOT NULL DEFAULT 0;
